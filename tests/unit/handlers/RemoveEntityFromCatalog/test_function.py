...
FN_NAME = 'RemoveEntityFromCatalog'
DATA_DIR = './data'
...
def mock_fn(
    ...
    import src.handlers.RemoveEntityFromCatalog.function as fn

    # NOTE: use mocker to mock any top-level variables outside of the handler function.
    mocker.patch(
        'src.handlers.RemoveEntityFromCatalog.function.JWT',
        mock_auth
    )

    mocker.patch(
        'src.handlers.RemoveEntityFromCatalog.function.CATALOG_ENDPOINT',
        mock_endpoint
    )
...
### Code Tests
def test_RemoveEntityFromCatalogError(
    mock_event_data: Entity,
    mock_fn: ModuleType
    ):
    '''Test RemoveEntityFromCatalogError class'''
    e = mock_fn.RemoveEntityFromCatalogError(mock_event_data)
    assert str(e) == 'Failed to remove entity from catalog: MockResource'

def test__remove_entity_from_catalog(
    mock_fn: ModuleType,
    mock_event_data: Entity,
    mock_endpoint: str,
    mock_auth: JwtAuth,
    ...
    r = mock_fn._remove_entity_from_catalog(mock_event_data, mock_auth)

    assert requests_mocker.called is True
    assert requests_mocker.call_count == 1
    assert r.ok is True
    assert r.request.method == 'DELETE'
...
def test__remove_entity_from_catalog_fails(
    mock_fn: ModuleType,
    mock_event_data: Entity,
    mock_auth: JwtAuth,
    requests_mocker: requests_mock.Mocker,
    ...
    with pytest.raises(mock_fn.RemoveEntityFromCatalogError):
        mock_fn._remove_entity_from_catalog(mock_event_data, mock_auth)
